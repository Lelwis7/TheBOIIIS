...
# Falls lokal: .streamlit/secrets.toml | Falls Cloud: App Settings -> Secrets
try:
    ICAL_URL = st.secrets["calendar"]["ical_link"]
    ADMIN_PASSWORD = st.secrets["auth"]["admin_password"]
except Exception:
    st.error("❌ Secrets nicht gefunden! Bitte ical_link und admin_password konfigurieren.")
    st.stop()
...
if page == "🗓️ Events":
    ...
    with col_abo:
        # iPhone Kalender-Abo Link (webcal://)
            ...
            for i in reversed(df.index):
                ...
                with st.expander(f"🗓️ {row['date']}{t_title} - {row['summary']}"):
                    ...
                    if st.button("Ich bin dabei! ✅", key=f"j_ev_{i}") and n_in:
                        new_p = pd.DataFrame([{"event": row['summary'], "name": n_in}])
